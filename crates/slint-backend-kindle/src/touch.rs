...
pub(crate) struct TouchInput {
    ...
    screen_width: f32,
    screen_height: f32,
    /// Conversion from the framebuffer's physical pixels to Slint's logical
    /// pixels. This keeps input aligned when the same layout is scaled up for
    /// higher-resolution Kindle panels.
    scale_factor: f32,
...
impl TouchInput {
    ...
    pub(crate) fn poll(&mut self, window: &MinimalSoftwareWindow) {
        while let Some(event) = self.read_event() {
            match (event.kind, event.code) {
                ...
                (EVENT_ABSOLUTE_AXIS, TOUCH_POSITION_X) if self.should_update_position() => {
                    self.x = (event.value as f32) * self.screen_width / self.max_x;
                }
                (EVENT_ABSOLUTE_AXIS, TOUCH_POSITION_Y) if self.should_update_position() => {
    ...
    /// Some controllers report the initial X/Y before their tracking ID. Keep
    /// those first-contact coordinates, but once a pointer is owned ignore
    /// coordinates from any secondary touch slot.
    fn should_update_position(&self) -> bool {
        self.tracked_slot.is_none() || self.is_tracked_slot()
    }

    fn logical_position(&self) -> LogicalPosition {
        LogicalPosition::new(self.x / self.scale_factor, self.y / self.scale_factor)
    }
