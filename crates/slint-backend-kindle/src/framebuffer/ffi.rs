...
// Oasis 2/3 (Kindle platform codename Zelda). This extends the Rex request
// with two driver timestamp fields, making the ioctl payload 88 bytes.
#[repr(C)]
...
// ioctl number for Oasis 2/3 (Zelda), whose update payload is 88 bytes.
...
#[cfg(test)]
mod tests {
    use super::{UpdateRequest, UpdateRequestRex, UpdateRequestZelda};

    #[test]
    fn kindle_update_request_layouts_match_kernel_abis() {
        assert_eq!(std::mem::size_of::<UpdateRequest>(), 72);
        assert_eq!(std::mem::size_of::<UpdateRequestRex>(), 80);
        assert_eq!(std::mem::size_of::<UpdateRequestZelda>(), 88);
    }
}
